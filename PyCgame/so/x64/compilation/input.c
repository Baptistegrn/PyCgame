...
void input_update(Gestionnaire *jeu, GestionnaireEntrees *entrees) {
    if (!jeu) {
        fprintf(stderr, "Erreur: jeu NULL dans input_update\n");
        return;
    }
    if (!entrees) {
        fprintf(stderr, "Erreur: entrees NULL dans input_update\n");
        return;
    }
    ...
    while (SDL_PollEvent(&event)) {
        ...
        float coeff_largeur = 1.0f;
        float coeff_hauteur = 1.0f;
        if (jeu->largeur != 0) coeff_largeur = (float)jeu->largeur_actuel / jeu->largeur;
        if (jeu->hauteur != 0) coeff_hauteur = (float)jeu->hauteur_actuel / jeu->hauteur;
        ...
        switch (event.type) {
            ...
            case SDL_CONTROLLERBUTTONDOWN:
                if (event.cbutton.button < SDL_CONTROLLER_BUTTON_MAX) {
                    ...
                    fprintf(stderr, "Erreur: scancode manette invalide (%d)\n", event.cbutton.button);
                }
                break;
            ...
            case SDL_KEYDOWN:
                if (event.key.keysym.scancode < SDL_NUM_SCANCODES) {
                    ...
                    fprintf(stderr, "Erreur: scancode KEYDOWN invalide (%d)\n", event.key.keysym.scancode);
                }
                break;
            ...
            case SDL_KEYUP:
                if (event.key.keysym.scancode < SDL_NUM_SCANCODES) {
                    ...
                    fprintf(stderr, "Erreur: scancode KEYUP invalide (%d)\n", event.key.keysym.scancode);
                }
                break;
    ...
    if (nom[0] == 'f' ) {
        if (strlen(nom) == 2) {
            ...
            if (num >= 1 && num <= 9) {
                ...
                fprintf(stderr, "Erreur: nom de touche F invalide (%s)\n", nom);
            }
            ...
            else fprintf(stderr, "Erreur: nom de touche F invalide (%s)\n", nom);
        } else {
            fprintf(stderr, "Erreur: nom de touche F invalide (%s)\n", nom);
        }
    }
...
JEU_API bool touche_juste_presse(Gestionnaire *jeu, const char *touche) {
    if (!jeu || !jeu->entrees) {
        fprintf(stderr, "Erreur: jeu ou entrees NULL dans touche_juste_presse\n");
        return false;
    }
...
JEU_API bool touche_enfoncee(Gestionnaire *jeu, const char *touche) {
    if (!jeu || !jeu->entrees) {
        fprintf(stderr, "Erreur: jeu ou entrees NULL dans touche_enfoncee\n");
        return false;
    }
    SDL_Scancode sc = scancode_depuis_nom(touche);
    if (sc == SDL_SCANCODE_UNKNOWN) return false;
...
JEU_API bool touche_mannette_juste_presse(Gestionnaire *jeu , const char *touche){
    if (!jeu || !jeu->entrees) {
        fprintf(stderr, "Erreur: jeu ou entrees NULL dans touche_manette_juste_presse\n");
        return false;
    }
...
JEU_API bool touche_mannette_enfoncee(Gestionnaire *jeu , const char *touche){
    if (!jeu || !jeu->entrees) {
        fprintf(stderr, "Erreur: jeu ou entrees NULL dans touche_manette_enfoncee\n");
        return false;
    }
    SDL_GameControllerButton sc = bouton_manette_depuis_nom(touche);
    if (sc ==SDL_CONTROLLER_BUTTON_INVALID ) return false;
